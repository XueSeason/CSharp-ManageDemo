...
namespace 机器数据管理
{
    public partial class filloutStorageForm : Form
    {
        data db = new data();
        data db2 = new data();
        public filloutStorageForm()
        {
            InitializeComponent();
        }
        ...
        private void filloutStorageForm_Load(object sender, EventArgs e)
        {
            //加载图像 设置界面显示
            Bitmap bmp = new Bitmap("水纹2.jpg");//打开一张图将起转换为24位
            ...
            string strsql = "select 产品编号 from 产品库存信息";//读取数据的SQL语句
            DataTable dt = new DataTable();
            dt = db.getTable(strsql);//调用读数据的函数进行读，并把读取的表返回。
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                comboBox1.Items.Add(dt.Rows[i]["产品编号"].ToString());
            }
        }
        ...
        {
            if (int.Parse(textBox5.Text) > int.Parse(label7.Text))
            {
                MessageBox.Show("库存量不足！");
            }
            else
            {
                string strsql = "insert into 出库信息表(产品编号,产品名称,出库管理员,出库时间,客户名,客户地址,客户联系电话,数量) values('" + comboBox1.Text + "'";
                ...
                {
                    strsql = "select * from 产品库存信息 where 产品编号='" + comboBox1.Text + "'";
                    DataTable dt = new DataTable();
                    dt = db2.getTable(strsql);
                    strsql = "update 出库信息表 set 密码1='" + dt.Rows[0]["密码1"].ToString() + "'";
                    strsql = strsql + ",密码2='" + dt.Rows[0]["密码2"].ToString() + "'";
                    strsql = strsql + ",密码3='" + dt.Rows[0]["密码3"].ToString() + "'";
                    strsql = strsql + " where 产品编号='" + comboBox1.Text + "'";
                    db.execSql(strsql);

                    strsql = "select 数量 from 产品库存信息 where 产品编号='" + comboBox1.Text + "'";
                    dt = new DataTable();
                    dt = db.getTable(strsql);
                    int num = int.Parse(label7.Text);
                    num -= int.Parse(textBox5.Text);
                    strsql = "update 产品库存信息 set 数量='" + num + "' where 产品编号='" + comboBox1.Text + "'";
                    db.execSql(strsql);
                }
                this.Close();
            }
        ...
        {
            string strsql = "select 产品名称,数量 from 产品库存信息 where 产品编号='"+comboBox1.Text+"'";
            DataTable dt = new DataTable();
            dt = db.getTable(strsql);
            textBox1.Text = dt.Rows[0]["产品名称"].ToString();
            label7.Text = dt.Rows[0]["数量"].ToString();
        }
    }
}
